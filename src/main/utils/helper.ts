...
export const charToHexa: Helperable["charToHexa"] = (s) =>
  `000${s.charCodeAt(0).toString(16)}`.slice(-4);

export const delay: Helperable["delay"] = async (ms) =>
...
export const expireAtToTtl: Helperable["expireAtToTtl"] = (expireAt) =>
  expireAt - Math.floor(Date.now() / 1000);

export const fromCallback: Helperable["fromCallback"] = async <T>(
...
export const mapSeriesAsync: Helperable["mapSeriesAsync"] = async <T, U>(
  array: T[],
  iterator: (item: T, index: number, array: T[]) => Promise<U>,
) => {
  const result: U[] = [];
  await eachAsync<T>(array, async (item, index) => {
    result.push((await iterator(item, index, array)) as U);
  });
  return result;
};

export const parseFlatJson: Helperable["parseFlatJson"] = (rawData) => {
  const flatData: Mapable<string> = JSON.parse(rawData);
  return unflatten(flatData);
};

export const removeDiacritics = (data: unknown): string => {
  if (typeof data !== "string" || !data) {
    return `${data}`;
  }
  return defaultDiacriticsRemovalMap.reduce(
    (data, { letters, base }) => data.replace(letters, base),
    data,
  );
};

export const repeat: Helperable["repeat"] = (
  count,
  iterator: (index) => void,
) => {
...
export const repeatAsync: Helperable["repeatAsync"] = async (
  count,
  iterator: (index: number) => Promise<void>,
) => {
  await mapSeriesAsync(Array.from(Array(count)), (__, index) =>
    iterator(index),
  );
};

export const staticImplements: Helperable["staticImplements"] =
  <T>() =>
...
export const validTtlToExpireAt: Helperable["validTtlToExpireAt"] = (
  ttl,
): number => Math.floor(Date.now() / 1000) + ttl;
